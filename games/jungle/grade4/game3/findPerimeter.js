...
const shapesData = [
  ...
  {
    name: "ხუთკუთხედი",
    instruction: "იპოვე ტოლგვერდა ხუთკუთხედის პერიმეტრი",
  },
];
...
function startRound() {
  ...
  if (shape.name === "ტოლგვერდა სამკუთხედი") {
    ...
    htmlCode = `
          ...
          `;
  } else if (shape.name === "ხუთკუთხედი") {
    let side = Math.floor(Math.random() * 10) + 3;
    correctAns = side * 5;

    htmlCode = `
            <svg class="perimeter-svg" viewBox="0 0 240 240">
              <polygon points="120,40 215,109 179,221 61,221 25,109" />
              <text x="105" y="240" class="side-label">${side} სმ</text>
            </svg>
          `;
  }
